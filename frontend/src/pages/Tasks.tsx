...
export default function SynchroDeskDashboard() {
  ...
  // Regex rules
  const TITLE_REGEX = /^.{3,50}$/; // Title: 3-50 characters
  const DESCRIPTION_REGEX = /^.{0,200}$/; // Description: max 200 characters
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100">
      <main className="p-6">
        ...
        <header className="flex flex-col md:flex-row justify-between mb-8">
          ...
          <p className="font-semibold bg-linear-to-r from-slate-700 via-slate-800 to-slate-900 bg-clip-text text-transparent mt-1">
            Here's your targets for today
          </p>
        </header>
        ...
        <div className="space-y-6 rounded-xl bg-white shadow-sm border border-gray-300 p-6 mt-10">
          <div className="flex items-center justify-between mb-6">
            ...
            <label className="opacity-80 px-2 py-2 text-end text-xs font-bold text-gray-800">
              You Can Only Add New Tasks To Workspaces In The Workspace Page
            </label>
          </div>
          ...
          {filteredWorkspaceTasks?.map((ws) => (
            <div key={ws.workspaceId} className="mb-6">
              {/* Workspace Header */}
              <h3 className="text-xl font-bold bg-linear-to-r from-slate-700 via-slate-800 to-slate-900 bg-clip-text text-transparent mb-3">
                {ws.workspaceName}
              </h3>
      ...
      {/* ADD TASK MODAL */}
      {/* Add Task Modal with Regex Validation */}
      {showTaskModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            <h2 className="text-2xl font-bold bg-linear-to-r from-slate-700 via-slate-800 to-slate-900 bg-clip-text text-transparent mb-4 text-center">
              Add New Task
            </h2>
            ...
            <textarea
              ...
              onChange={(e) => {
                ...
                setTaskErrors((prev) => ({
                  ...prev,
                  description: !DESCRIPTION_REGEX.test(value),
                }));
              }}
              className={`w-full p-3 border rounded mb-2 ${
            ...
            <input
              ...
              onChange={(e) => {
                const value = e.target.value;
                setNewTaskData({ ...newTaskData, dueDate: value });
                setTaskErrors((prev) => ({
                  ...prev,
                  dueDate: value ? !DATE_REGEX.test(value) : false,
                }));
              }}
              className={`w-full p-3 border rounded mb-2 ${
                taskErrors.dueDate ? "border-red-500" : "border-gray-300"
              }`}
            />
            {taskErrors.dueDate && (
              <p className="text-red-500 text-xs mb-2">Invalid date format</p>
            )}
              ...
              <button
                ...
                className={`flex-1 p-3 font-bold rounded ${
                  ...
                  taskErrors.dueDate
                    ...
                    : "bg-linear-to-r from-slate-700 via-slate-800 to-slate-900 shadow-xl border-b border-slate-600 text-white"
                }`}
              >
                Add Task
              </button>
      ...
      {/* Edit Task Modal with Regex Validation */}
      {editTaskData && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            <h2 className="text-2xl font-bold bg-linear-to-r from-slate-700 via-slate-800 to-slate-900 bg-clip-text text-transparent mb-4 text-center">
            ...
            <input
              ...
              onChange={(e) => {
                const value = e.target.value;
                setEditTaskData({ ...editTaskData, title: value });
                setEditErrors((prev) => ({
                  ...prev,
                  title: !TITLE_REGEX.test(value),
                }));
            ...
            <textarea
              ...
              onChange={(e) => {
                const value = e.target.value;
                setEditTaskData({ ...editTaskData, description: value });
                setEditErrors((prev) => ({
                  ...prev,
                  description: !DESCRIPTION_REGEX.test(value),
                }));
            ...
            <input
              ...
              onChange={(e) => {
                const value = e.target.value;
                setEditTaskData({ ...editTaskData, dueDate: value });
                setEditErrors((prev) => ({
                  ...prev,
                  dueDate: value ? !DATE_REGEX.test(value) : false,
                }));
